...
try{
const resp=await fetch(`api/get_profile_contratante.php?user_id=${userIdToFetch}`);
const data=await resp.json();
if(!resp.ok||!data.success){
console.warn('Perfil não encontrado no backend ou erro na API. Exibindo dados padrão.');
editableFields.athleteName.view.textContent='Nome do Contratante Padrão';
editableFields.username.view.textContent='usuario_padrao';
editableFields.bio.view.textContent='Bio padrão do contratante.';
editableFields.sport.view.textContent='Esporte Padrão';
return;
}
const profile=data.profile;
editableFields.athleteName.view.textContent=profile.nome_completo||profile.usuario||'Nome do Contratante';
...
profilePicture.src='caminho/para/imagem/padrao.png';
}
...
try{
const resp=await fetch('api/update_profile_contratante.php',{
method:'POST',
body:formData
});
const data=await resp.json();
...
formData.append('user_id',currentUserId);
try{
const response=await fetch('api/create_post_contratante.php',{
method:'POST',
body:formData
});
...
mediaElement.src='tcc/TCC/'+postData.mediaUrl;
...
const userIdToFetch=(!currentUserId||currentUserId==='null')?1:currentUserId;
try{
const response=await fetch(`api/get_posts_by_user_contratante.php?user_id=${userIdToFetch}`);
...
mediaElement=document.createElement('img');
mediaElement.src='tcc/TCC/'+post.mediaUrl;
...
console.log('ID da publicação a ser excluída:',postId);
const formData=new FormData();
formData.append('post_id',postId);
try{
// CORRIGIDO: O caminho deve ser relativo ao arquivo JS
        const response = await fetch('delete_foto_contratante.php', { 
            method: 'POST',
            body: formData
        });
        const result = await response.json();
...
saveEditedPostBtn.addEventListener('click', async () => {
    ...
    try {
        const response = await fetch('api/update_post_contratante.php', {
            method: 'POST',
            body: formData
        });
        const result = await response.json();
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (storedUserId) {
        ...
        console.log('ID do usuário a ser carregado:', window.currentUserId);
    } else {
        console.error('Nenhum ID de usuário encontrado. Carregando dados padrão.');
